...
export const view = async (req: Request, res: Response) => {
	try {
		let allBigRigServices = await BigRigService.find(req.body)
			.sort({ created_at: 1 })
			.exec();
	...
	try {
		...
		if (newBigRigService) {
			...
			var data = await ejs.renderFile(
				"src/utils/emailTemplates/bigRigServiceForm.ejs",
				{
					franchise: "Franchise Name",
					group_id: bigRigService.group_id,
					owner_id: bigRigService.owner_id,
					type: bigRigService.type,
					truck_id: bigRigService.truck_id,
					location: bigRigService.location,
					service_hours: bigRigService.service_hours,
				}
			);
			sendInspection(data);
		} else {
...
export const update = async function (req: Request, res: Response) {
	try {
		const data = { ...req.body };
		let updatedBigRigService = await BigRigService.findByIdAndUpdate(
			req.body._id,
			data,
