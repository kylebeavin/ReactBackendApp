...
export const view = async (req: Request, res: Response) => {
	try {
		let allKenworthDealers = await KenworthDealer.find(req.body)
			.sort({ created_at: 1 })
			.exec();
	...
	try {
		...
		if (newKenworthDealer) {
			...
			var data = await ejs.renderFile(
				"src/utils/emailTemplates/kenworthDealerForm.ejs",
				{
					franchise: "Franchise Name",
                    group_id : kenworthDealer.group_id,
                    owner_id : kenworthDealer.owner_id,
                    type : kenworthDealer.type,
                    truck_id : kenworthDealer.truck_id,
                    contact : kenworthDealer.contact,
                    location : kenworthDealer.location,
                    website : kenworthDealer.website,
                    phone : kenworthDealer.phone,
                    service_hours : kenworthDealer.service_hours,
				}
			);
			sendInspection(data);
		} else {
...
export const update = async function (req: Request, res: Response) {
	try {
		const data = { ...req.body };
		let updatedKenworthDealer = await KenworthDealer.findByIdAndUpdate(
			req.body._id,
			data,
